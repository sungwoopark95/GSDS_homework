...
Tensor* Softmax::backward(Tensor* out){
  ...
  for(int n=0; n<N; ++n){
    ...
    double dL0 = out -> getGrad(n, 0);
    double dL1 = out -> getGrad(n, 1);
    double dL2 = out -> getGrad(n, 2);

    grad_val[0] = (dL0*p0*(1-p0)) - (dL1*p0*p1) - (dL2*p0*p2);
    grad_val[1] = (dL1*p1*(1-p1)) - (dL0*p1*p0) - (dL2*p1*p2);
    grad_val[2] = (dL2*p2*(1-p2)) - (dL1*p2*p1) - (dL0*p2*p0);
...
Tensor* Linear::backward(Tensor* out){	
  ...
  for(int n=0; n<N; ++n){
    for(int i=0; i<ic_; ++i){
      ...
      // TODO: calculate tmp
      for(int o=0; o<oc_; ++o){
        tmp += (out_->getGrad(n, o) * weight_->getData(i, o));
      }
  ...
  for(int i=0; i<ic_; ++i){
    for(int o=0; o<oc_; ++o){
      ...
      for(int n=0; n<N; ++n){
        tmp_grad += (out_->getGrad(n, o) * in_->getData(n, i));
        // tmp_grad += (in_->getData(n, o));
      }
...
void Loss::backward(Tensor *out){
  double dloss = 1.0;
  double grad, grad0, grad1, grad2;

  // TODO: calculate grad and set gradient of 'out' tensor
  // int N = out->getN();
  for (int n=0;n<n_;++n) {

    grad0 = (double)(-1.0) * dloss * ctx_[n*6+0] * (1.0 / ctx_[n*6+3]);
    grad1 = (double)(-1.0) * dloss * ctx_[n*6+1] * (1.0 / ctx_[n*6+4]);
    grad2 = (double)(-1.0) * dloss * ctx_[n*6+2] * (1.0 / ctx_[n*6+5]);

    out->setGrad(n, 0, grad0/(double)n_);
    out->setGrad(n, 1, grad1/(double)n_);
    out->setGrad(n, 2, grad2/(double)n_);
